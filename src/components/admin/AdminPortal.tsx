...
const AdminPortal: React.FC = () => {
  ...
  const renderAccounts = () => (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              <span className="text-gray-600">Total Due:</span>
              <span className="font-bold text-orange-600">SAR 890</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Paid:</span>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200">
        <div className="p-6 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-[#1a3a5f]">Payment Details</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="text-left py-3 px-6 text-sm font-medium text-gray-500">Recipient</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {allBookings.slice(0, 10).map((booking) => (
                <tr key={booking.id} className="hover:bg-gray-50">
                  <td className="py-4 px-6 text-sm font-medium text-[#1a3a5f]">#{booking.id.slice(-6)}</td>
                  <td className="py-4 px-6 text-sm text-gray-900">Driver Payment</td>
                  <td className="py-4 px-6 text-sm text-gray-900">{booking.driver?.name || 'Unassigned'}</td>
                  <td className="py-4 px-6 text-sm font-bold text-[#1a3a5f]">SAR {(booking.price * 0.8).toFixed(0)}</td>
                  <td className="py-4 px-6">
                  ...
                  <td className="py-4 px-6">
                    <button 
                      onClick={() => alert(`Marking payment as paid for booking #${booking.id.slice(-6)}`)}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden my-auto">
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <h2 className="text-2xl font-bold text-[#1a3a5f]">
                Create {createType === 'booking' ? 'New Booking' : createType.charAt(0).toUpperCase() + createType.slice(1)}
              </h2>
              <button
                ...
                <form className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Leave blank or enter 0 for no commission. Admin can edit this later.
                    </p>
                  </div>
                </form>
              )}
